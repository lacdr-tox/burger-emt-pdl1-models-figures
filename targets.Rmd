...
kmu_sens_plot_data %>%
  ...
  ggplot(aes(`# Values[S].InitialValue` / 1e5, `[Z]`, linetype = stable, 
             color = as.factor(change), group = interaction(interval, k_mu2)))+ 
  geom_line(size = 0.5) +
  scale_linetype_discrete(guide = "none", limits = c(T,F)) +
...
bifs_snail_data %>% 
  filter(experiment == "no_int") %>%
  add_column(k_mu2 = as.factor(0.05)) -> tcs_bif

pp +
  geom_line(data = tcs_bif, color = "black", size = 1)
...
list(
  tar_target(si_kmu_sens_file, {
    ...
    ggsave(path, plot = kmu_sens_plot, width = 10, height = 5)
  })
)
```
