...
list(
  ...
  tar_target(emt_ifn0_file, path(emt_td_path, "EMT no IFN fluxes.txt"), format = "file"),
  tar_target(emt_ifn1_file, path(emt_td_path, "EMT IFN fluxes.txt"), format = "file"),
  tar_target(met_no_int_file, path(emt_td_path, "MET no interaction fluxes.txt"), format = "file"),
  tar_target(met_ifn0_file, path(emt_td_path, "MET no IFN fluxes.txt"), format = "file"),
  tar_target(met_ifn1_file, path(emt_td_path, "MET IFN fluxes.txt"), format = "file")
)
```
...
time_dynamics_long %>%
  ...
  pivot_longer(cols = c(-type, -experiment, -time)) -> time_dynamics_long_flux

td_vars <- c("Total", td_vars)
td_labels <- c(
...
time_dynamics_long_flux %>% 
  filter(name %in% td_vars) %>%
  mutate(name = factor(name, levels = td_vars, labels = td_labels)) %>%
  mutate(experiment = factor(experiment, levels = exp_order)) %>%
...
```{targets si_td_mPY_file}
  tar_target(si_td_mPY_file, {
    ...
    ggsave(path, plot = si_td_mPY, width = 12, height = 10, device = cairo_pdf)
  })
```
